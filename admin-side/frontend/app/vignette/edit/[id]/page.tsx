...
import { Vignette } from "@/components/vignette/VignetteTable"
...
export default function EditVignettePage() {
  const { id } = useParams()
  ...
  const [vignette, setVignette] = useState<Vignette>();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch vignette data
        const vignetteResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/vignettes/${id}`, {
          headers: {
        ...
        const vignetteResult = await vignetteResponse.json()

        if (!vignetteResponse.ok) {
          throw new Error(vignetteResult.message || "Erreur lors de la récupération de la vignette")
        }

        const vignette = vignetteResult.data
        setVignette(vignetteResult.data)
        // console.log(vignette)

        form.reset({
          vehicule_id: vignette.vehicule_id.toString(),
          numero: vignette.numero,
          date: new Date(vignette.date),
          date_prochaine: new Date(vignette.date_prochaine),
          date_reglement: vignette.date_reglement ? new Date(vignette.date_reglement) : undefined,
          prix: vignette.prix.toString(),
          description: vignette.description || "",
        })
        // console.log(form.getValues())
        // Load existing documents
        if (vignette.fichiers) {
          try {
            const docs = JSON.parse(vignette.fichiers)
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({
        title: "Succès",
        description: "Vignette mise à jour avec succès!",
      })
  ...
  return (
    <Layout>
      <div className="max-w-8xl mx-auto p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold">Modifier la vignette</h1>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar
                          ...
                          disabled={(date) => {
                            ...
                            const vignetteDate = vignette?.date ? new Date(vignette.date) : null;
