...
type VidangeFormProps = {
  onSuccess?: () => void
  initialData?: VidangeData
  isEditMode?: boolean
}
...
export default function VidangeForm({ onSuccess, initialData, isEditMode = false }: VidangeFormProps) {
  ...
  const getDefaultValues = () => ({
    ...
    kilometrage_actuel: initialData?.kilometrage_actuel?.toString() || "",
    kilometrage_prochain: initialData?.kilometrage_prochain?.toString() || "",
  ...
  useEffect(() => {
    const getImageUrl = (docPath: string) => {
      ...
      return `${process.env.NEXT_PUBLIC_API_URL}/storage/vidanges/documents/${filename}`
    }
    if (isEditMode && initialData?.fichier) {
      try {
        const docs = typeof initialData.fichier === "string" ? JSON.parse(initialData.fichier) : initialData.fichier
  ...
  const handleDocumentsChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (validFiles.length > 0) {
        ...
        form.setValue("fichier", [...documents, ...newDocs])
      }
      ...
      try {
        ...
        const url =
          isEditMode && initialData?.id
            ? `${process.env.NEXT_PUBLIC_API_URL}/api/vidanges/${initialData.id}`
            : `${process.env.NEXT_PUBLIC_API_URL}/api/vidanges`
        ...
        toast({
          title: "Succès",
          description: isEditMode ? "Vidange mise à jour avec succès!" : "Vidange créée avec succès!",
        })
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6" encType="multipart/form-data">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Kilométrage prochain */}
          <FormField
            control={form.control}
            name="kilometrage_prochain"
            render={({ field }) => (
              <FormItem>
                <FormLabel>Kilométrage prochain *</FormLabel>
                <FormControl>
                  <Input type="number" min="0" placeholder="0" {...field} disabled={loading} />
                </FormControl>
                <FormMessage />
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={loading}>
            {loading ? "Traitement..." : isEditMode ? "Mettre à jour" : "Créer la vidange"}
          </Button>
