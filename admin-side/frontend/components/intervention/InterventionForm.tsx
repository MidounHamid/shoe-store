...
export default function InterventionForm({ onSuccess, initialData, isEditMode = false }: InterventionFormProps) {
  ...
  useEffect(() => {
    const getImageUrl = (docPath: string) => {
      ...
      return `${process.env.NEXT_PUBLIC_API_URL}/storage/interventions/documents/${filename}`
    }
      ...
      try {
        ...
        const url =
          isEditMode && initialData?.id
            ? `${process.env.NEXT_PUBLIC_API_URL}/api/interventions/${initialData.id}`
            : `${process.env.NEXT_PUBLIC_API_URL}/api/interventions`
        ...
        toast({
          title: "Succès",
          description: isEditMode ? "Intervention mise à jour avec succès!" : "Intervention créée avec succès!",
        })
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6" encType="multipart/form-data">
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={loading}>
            {loading ? "Traitement..." : isEditMode ? "Mettre à jour" : "Créer l'intervention"}
          </Button>
